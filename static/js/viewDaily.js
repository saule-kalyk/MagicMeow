document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderDay() {
        ...
        weekRangeEl.textContent = String(day).padStart(2, '0');
        currentDayDisplay.textContent = `${monthNames[month]} ${day}, ${year}`;
    ...
    function addPlanToDay(plan) {
        const { date_start, date_end, plan_name, time_start, time_end, quadrant, id, completed } = plan;
        ...
        if (startDate.toDateString() === current.toDateString()) {
            if (time_start && time_end) {
                ...
                for (let hour = Math.floor(startTime); hour <= Math.floor(endTime); hour++) {
                    const slot = document.querySelector(`.plan-slot[data-hour="${hour}"]`);
                    if (slot) {
                        ...
                        if (hour === Math.floor(startTime) && hour === Math.floor(endTime)) {
                            timeSpan.textContent = `${time_start} - ${time_end}`;
                        } else if (hour === Math.floor(startTime)) {
                            timeSpan.textContent = `${time_start} - ${String(hour + 1).padStart(2, '0')}:00`;
                        } else if (hour === Math.floor(endTime)) {
                            timeSpan.textContent = `${String(hour).padStart(2, '0')}:00 - ${time_end}`;
                        } else {
                            timeSpan.textContent = `${String(hour).padStart(2, '0')}:00 - ${String(hour + 1).padStart(2, '0')}:00`;
                        }

                        planElement.appendChild(checkbox);
                        planElement.appendChild(name);
                        planElement.appendChild(timeSpan);
