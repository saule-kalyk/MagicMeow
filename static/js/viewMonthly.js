document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderCalendar(date) {
        ...
        for (let i = 0; i < totalCells; i++) {
            ...
            const dateString = `${year}-${String(cellDate.getMonth() + 1).padStart(2, '0')}-${String(cellDate.getDate()).padStart(2, '0')}`;
            ...
            if (dayCounter > 0 && dayCounter <= daysInMonth) {
                ...
                fadedNumber.classList.add("day-number", "other-month");
    ...
    // Fetch user's plans and display them on the calendar
    async function fetchPlans() {
        try {
            ...
            if (response.ok && result.success) {
                ...
                document.querySelectorAll('.plan').forEach(plan => plan.remove());
                plans.forEach(plan => {
    ...
    function addPlanToCalendar(plan) {
        ...
        let endDate = date_end ? new Date(date_end) : startDate;
        ...
        while (current <= endDate) {
            ...
            if (cell) {
                ...
                if (!planWrapper.querySelector(`[data-plan-id="${id}"]`)) {
                    ...
                    planElement.classList.add("plan-quadrant", quadrant);
                    ...
                    planWrapper.appendChild(planElement);
                }
            }
...
function addPlanToMonth(plan) {
    ...
    while (current <= endDate) {
        if (current.getMonth() === currentMonth && current.getFullYear() === currentYear) {
            ...
            const dayBox = document.querySelector(`.calendar-day[data-day="${day}"] .plans`);
